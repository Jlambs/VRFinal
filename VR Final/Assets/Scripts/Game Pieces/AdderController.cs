...
{
    public bool isAdder = true;  // false means subtractor

    public GameObject adderObject;
    ...
    {
        RYB = RYBvalues;
        adderObject.GetComponent<MeshRenderer>().material.color = RYBToColor(RYBvalues);
    }

    public void OperateOnMarble(GameObject marble)
    {
        if (isAdder)
        {
            marble.GetComponent<MarbleController>().AddColor(RYB);
        }
        else
        {
            marble.GetComponent<MarbleController>().SubtractColor(RYB);
        }
    }
}
