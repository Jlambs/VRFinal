...
public class GeneratorController : MonoBehaviour
{
    ...
    public GameObject generationChamber;
	...
	{
        // Create one marble and color according to the current RBY value
        GameObject newMarble = Instantiate(marblePrefab, transform.position + new Vector3(0, 3, 0), Quaternion.identity);
        newMarble.GetComponent<MeshRenderer>().material.color = RYBToColor(RYB);
	}

	public void SetColorFromRYB(int[] RYBvalues)
	{
        RYB = RYBvalues;
		endcap.GetComponent<MeshRenderer>().material.color = RYBToColor(RYBvalues);
        generationChamber.GetComponent<MeshRenderer>().material.color = RYBToColor(RYBvalues);
    }
}
